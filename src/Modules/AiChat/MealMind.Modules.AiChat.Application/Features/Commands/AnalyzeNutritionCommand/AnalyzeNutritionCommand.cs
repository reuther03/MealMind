...
namespace MealMind.Modules.AiChat.Application.Features.Commands.AnalyzeNutritionCommand;

public record AnalyzeNutritionCommand(
    [property: JsonIgnore]
    Guid ConversationId,
    string Prompt,
    int? Weeks = null
) : ICommand<StructuredResponse>
{
    public sealed class Handler : ICommandHandler<AnalyzeNutritionCommand, StructuredResponse>
    {
        ...
        public Handler(INutritionSummaryService nutritionSummaryService, IConversationRepository conversationRepository, IDocumentRepository documentRepository,
            IAiChatUserRepository aiChatUserRepository, IUserService userService, IEmbeddingService embeddingService, IAiChatService responseManager,
        ...
        public async Task<Result<StructuredResponse>> Handle(AnalyzeNutritionCommand request, CancellationToken cancellationToken)
        {
            var aiUser = await _aiChatUserRepository.GetByUserIdAsync(_userService.UserId!, cancellationToken);
            if (aiUser is null)
                return Result<StructuredResponse>.NotFound("AI Chat user not found.");

            var conversation = await _conversationRepository.GetByIdAsync(request.ConversationId, cancellationToken);
            if (conversation is null)
                return Result<StructuredResponse>.NotFound("Conversation not found.");
            ...
            var nutritionSummary = await _nutritionSummaryService.BuildSummaryAsync(aiUser.Id, request.Weeks, cancellationToken);
            ...
            var response = await _responseManager.GenerateStructuredResponseWithNutritionSummaryAsync(
                request.Prompt,
                documentsText,
                nutritionSummary,
                chatMessages,
