...
public class DeleteImageAnalyzeJob : BackgroundService
{
    ...
    private async Task ProcessDeleteImageAnalyzeAsync(CancellationToken cancellationToken)
    {
        ...
        var imageAnalyzesToDelete = await dbContext.FoodImageAnalyzeSessions
            .Where(x =>
                (x.ImageAnalyze.SavedAt != null && x.ImageAnalyze.SavedAt.Value <= today.AddDays(-7)) ||
                (x.ImageAnalyze.SavedAt == null && x.CreatedAt <= today.AddDays(-14))
            ).ToListAsync(cancellationToken);

        if (imageAnalyzesToDelete.Count != 0)
        {
            dbContext.FoodImageAnalyzeSessions.RemoveRange(imageAnalyzesToDelete);
