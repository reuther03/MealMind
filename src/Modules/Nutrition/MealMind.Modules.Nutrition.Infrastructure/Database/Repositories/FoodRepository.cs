...
internal class FoodRepository : Repository<Food, NutritionDbContext>, IFoodRepository
{
    ...
    public async Task<Food?> GetByBarcodeAsync(string barcode, CancellationToken cancellationToken)
        => await _context.Foods.FirstOrDefaultAsync(x => x.Barcode == barcode, cancellationToken);
}